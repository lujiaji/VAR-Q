...
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default='Benchmark/DPG/dpg_bench_updated.csv',
    )
    parser.add_argument(
        "--res-path",
        type=str,
...
class MPLUG(torch.nn.Module):
    def __init__(self, ckpt='YOUR_PATH/modelscope/mplug_vqa/iic/mplug_visual-question-answering_coco_large_en', device='gpu'):
        super().__init__()
...
def prepare_dpg_data(args):
    ...
    for i, line in data.iterrows():
        ...
        try:
            ...
            qid = 0
        dep = line.dependency
        if not isinstance(dep, str) or dep.strip() == "" or dep != dep:
