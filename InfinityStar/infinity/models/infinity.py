...
class Infinity(nn.Module):
    def __init__(
        ...
        self.last_kv_cache_bytes = 0
        self.last_kv_cache_mb = 0.0
        self.runtime_mem_trace = []
        self.runtime_latency_trace = []
        self._weights_baseline_mb = 0.0
        ...
        for _ in range(depth):
            block_idx = len(self.unregistered_blocks)
            block = SelfAttnBlock(
                embed_dim=self.C, 
                cond_dim=self.D,
                num_heads=num_heads, 
                ...
                q_bits=getattr(other_args, 'q_bits', 8),
                quant_method=getattr(other_args, 'quant_method', 'G_SCALE_HEAD_DIM'),
                qkv_format=getattr(other_args, 'qkv_format', 'BHLc'),
                enable_quantization=bool(getattr(other_args, 'enable_quantization', 0)),
                rescale_qk=bool(getattr(other_args, 'rescale_qk', 0)),
                enable_fused_kv_flashattn=bool(getattr(other_args, 'enable_fused_kv_flashattn', 0)),
                enable_sageattn=bool(getattr(other_args, 'enable_sageattn', 0)),
                sageattn_type=getattr(other_args, 'sageattn_type', 'sageattn'),
            )
            block.attn.debug_block_id = block_idx
            # block.bfloat16()
    ...
    def _estimate_live_kv_cache_bytes(self) -> int:
        total_bytes = 0
        ...
        for block in self.unregistered_blocks:
            ...
            # Quantized KV cache path.
            k_varq = getattr(attn, "k_varq", None)
            v_varq = getattr(attn, "v_varq", None)
            if k_varq is not None and v_varq is not None:
                try:
                    k_bytes = int((k_varq.cache_bytes() or {}).get("total_bytes", 0))
                    v_bytes = int((v_varq.cache_bytes() or {}).get("total_bytes", 0))
                    total_bytes += k_bytes + v_bytes
                    continue
                except Exception:
                    pass

            # Fallback cache containers (dict/list of tensors).
            for cache_name in ("cached_k", "cached_v"):
                cache_obj = getattr(attn, cache_name, None)
    ...
    def _reset_runtime_mem_trace(self) -> None:
        self.runtime_mem_trace = []

    def _reset_latency_trace(self) -> None:
        self.runtime_latency_trace = []

    def _enable_latency_recording(self):
        rec = {'attn': 0.0, 'ffn': 0.0}
        for block in self.unregistered_blocks:
            block._latency_recorder = rec
        return rec

    def _disable_latency_recording(self):
        for block in self.unregistered_blocks:
            block._latency_recorder = None

    def _record_runtime_mem_point(self, point_name: str, scale_ind=None, repeat_idx=None) -> None:
        if not torch.cuda.is_available():
            return
        try:
            ts_ns = time.perf_counter_ns()
            alloc_mb = float(torch.cuda.memory_allocated()) / (1024.0 * 1024.0)
            reserved_mb = float(torch.cuda.memory_reserved()) / (1024.0 * 1024.0)
            max_alloc_mb = float(torch.cuda.max_memory_allocated()) / (1024.0 * 1024.0)
            max_reserved_mb = float(torch.cuda.max_memory_reserved()) / (1024.0 * 1024.0)
            kv_bytes = self._estimate_live_kv_cache_bytes()
            kv_mb = float(kv_bytes) / (1024.0 * 1024.0)
            self.runtime_mem_trace.append(
                {
                    "ts_ns": int(ts_ns),
                    "point": str(point_name),
                    "scale_ind": scale_ind,
                    "repeat_idx": repeat_idx,
                    "alloc_mem_mb": alloc_mb,
                    "reserved_mem_mb": reserved_mb,
                    "max_alloc_mem_mb": max_alloc_mb,
                    "max_reserved_mem_mb": max_reserved_mb,
                    "kv_cache_mb": kv_mb,
    ...
    def ar_infer_infinity_elegant(
        ...
        **kwargs,
        ...
        self._weights_baseline_mb = float(torch.cuda.memory_allocated()) / (1024.0 * 1024.0)
        for b in self.unregistered_blocks: b.attn.kv_caching(True)
        self._record_runtime_mem_point("kv_cache_enabled")
        ...
        _text_total = _text_t1 - _text_t0
        self.runtime_latency_trace.append({
            'scale_ind': 't0', 'repeat_idx': 0, 'scale_shape': None,
            'attn_time_s': _rec['attn'], 'ffn_time_s': _rec['ffn'],
            'others_time_s': _text_total - _rec['attn'] - _rec['ffn'],
            'total_time_s': _text_total,
            'allocated_mb': float(torch.cuda.memory_allocated()) / (1024.0 * 1024.0),
            'reserved_mb': float(torch.cuda.memory_reserved()) / (1024.0 * 1024.0),
            'kv_cache_mb': float(self._estimate_live_kv_cache_bytes()) / (1024.0 * 1024.0),
        })
        self._record_runtime_mem_point("after_text_prefill", scale_ind="t0", repeat_idx=0)
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for repeat_idx in range(infer_repeat_times):
                ...
                _step_total = _step_t1 - _step_t0
                self.runtime_latency_trace.append({
                    'scale_ind': si, 'repeat_idx': repeat_idx,
                    'scale_shape': tuple(pn),
                    'attn_time_s': _rec['attn'], 'ffn_time_s': _rec['ffn'],
                    'others_time_s': max(0.0, _step_total - _rec['attn'] - _rec['ffn']),
                    'total_time_s': _step_total,
                    'allocated_mb': float(torch.cuda.memory_allocated()) / (1024.0 * 1024.0),
                    'reserved_mb': float(torch.cuda.memory_reserved()) / (1024.0 * 1024.0),
                    'kv_cache_mb': float(self._estimate_live_kv_cache_bytes()) / (1024.0 * 1024.0),
                })
        ...
        self._record_runtime_mem_point("before_kv_cache_disable")
        self.last_kv_cache_bytes = self._estimate_live_kv_cache_bytes()
        self.last_kv_cache_mb = float(self.last_kv_cache_bytes) / (1024.0 * 1024.0)
        for b in self.unregistered_blocks: b.attn.kv_caching(False)
        self._record_runtime_mem_point("after_kv_cache_disable")
