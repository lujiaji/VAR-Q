...
def save_video(ndarray_image_list, fps=24, save_filepath='tmp.mp4'):
    if len(ndarray_image_list) == 1:
        ...
        try:
            ...
            print(f"imageio writer unavailable, fallback to cv2.VideoWriter: {e}")
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")
            writer = cv2.VideoWriter(save_filepath, fourcc, fps, (w, h))
            if not writer.isOpened():
                raise RuntimeError(f"Failed to open video writer for {save_filepath}")
            for frame in ndarray_image_list:
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--q_bits', type=int, default=8, choices=[2,4,8])
    parser.add_argument('--quant_method', type=str, default='G_SCALE_HEAD_DIM', choices=['G_TENSOR','G_SCALE_HEAD_DIM','G_HEAD_DIM','G_SCALE','G_TOKEN','G_TOKEN_HEAD'])
